...
In a Recurrent Neural Network (RNN), prior inputs are used to inform future outputs. As in all neural networks, input vectors are modified by weighted nodes as they travel through the network.

In a RNN, there is an additional hidden state which represents the context based on prior inputs. The hidden state is updated as inputs are fed through the network. This leads to the interesting property that the same input could produce a different output depending on the previous inputs given to the RNN.
...
\begin{enumerate}
	\item Vanilla Recurrent Neural Network
	\item Vanilla Gated Recurrent Unit
	\item Long Short-term Memory \textit{(LSTM)}
...
\subsubsection{Analysis of Main Features}

\begin{center}
	\textbf{Figure -a}: $F_1$ scores of machine learning classifiers, trained on BOW and TF-IDF vectors
\end{center}
...
\noindent In our experiments with traditional vectorisation approaches (\textit{figure \_a}), logistic regression classifiers and support Vector Machines produced the best performing models with the highest $F_1$ scores. The best result of 0.775 came from a Support Vector Machine, trained on Amazon reviews as TF-IDF vectors.
...
\vspace{10pt}
\subsubsection{Analysis of Miscellaneous Features}
...
\begin{center}
	\begin{tabular}{ |p{3cm}||p{3.7cm}|p{2cm}|  }
		\hline
