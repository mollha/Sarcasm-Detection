...
\begin{enumerate}[leftmargin=0cm]
	...
	\item \textbf{Support Vector Machine (SVM) --} An SVM uses a kernel function to draw a hyperplane which divides a feature space into two subspaces - separating positive and negative features. Features at the shortest euclidean distance (\textit{margin}) to the hyperplane are known as support vectors, and their distance from the hyperplane is maximised to create a decisive boundary between the sarcastic and non-sarcastic data.
