...
\begin{center}
	\textbf{Table 1}: Definition of Irony and Sarcasm\vspace{-5pt}
\end{center}
\begin{center}
...
\noindent A number of na\"{i}ve approaches have been used in previous research, where text is classified based on a set of linguistic rules. Rule-based techniques are fairly primitive when compared to their modern, deep learning counterparts, as each rule-set must be generated manually for each unique dataset. An early example of a rule-based approach is displayed in  Veale and Hao (2010a) \cite{veale2010detecting}, yielding an $F_1$ score of 0.73 in the irony detection task. They collect a corpus of 15039 phrases that match the wildcarded query "* as a *", applying a nine-step procedure which filters ironic similes out of the corpus at each stage. 

Maynard and Greenwood (2014) \cite{maynard2014cares} proposed that the sentiment contained in hashtags can indicate the presence of sarcasm in tweets, such as \#notreally in the example \textit{"I love doing the washing-up \#notreally"}. They used a rule-based approach, whereby if the sentiment of a tweet contradicts that of the hashtag, it is labelled sarcastic. They reported an $F_{1}$ score of 91.03\% on a random sample consisting of 400 tweets.

Bharti et al. (2015) \cite{bharti2015parsing} proposed two rule-based approaches - the first approach (IWS) classifies tweets that begin with injerjections, such as \textit{"wow"} and \textit{"yay"}, as sarcastic, and the second approach uses a parsing based lexical generation algorithm (PBLGA) to recognise sentiment-bearing situation phrases juxtaposed with contrasting sentiment phrases. When tested on 1500 tweets containing \#sarcasm, the IWS and PBLGA approaches yielded $F_1$ scores of 0.90 and 0.84, respectively.

\subsection{Machine-Learning Classifiers}\vspace{-10pt}
\noindent Machine learning algorithms do not rely upon a static linguistic rule-set. Instead, they can learn patterns that are associated with each class, allowing them to separate unseen data into these categories. There are a number of commonly used architectures, including Na\"{i}ve Bayes classifiers, Support Vector Machines (SVMs) and Logistic Regression classifiers, although not all approaches fit neatly into these categories.
...
In \textit{Poria et al. (2017)} \cite{poria2016deeper}, cited as the first study to use deep learning for sarcasm
...
\begin{center}
	\begin{tabular}{ |c||c|c|c|c|}
		...
		\textbf{Machine Learning Model}& \textbf{Feature(s)*} & \textbf{Precision} & \textbf{Recall} & \textbf{$\mathbf{F_1}$ Score}\\
		\hline\hline
		Support Vector Machine & TF-IDF & 0.   & 0. & 0.775\\
		K-Nearest Neighbours &  ElMo & 0.   & 0. & 0.685\\
		Support Vector Machine & BOW  & 0.   & 0. & 0.736\\
		Logistic Regression & ElMo  & 0.   & 0. & 0.809\\
		Logistic Regression & ElMo + S + T & 0.   & 0. & \textbf{0.818}\\
		\hline\hline
		\textbf{Deep Learning Model}& \textbf{Feature(s)} & \textbf{Precision} & \textbf{Recall} & \textbf{$\mathbf{F_1}$ Score}\\
		\hline
		CNN & GloVe  & 0.   & 0. & 0.827\\
		Vanilla RNN & ElMo  & 0.   & 0. & 0.812\\
		LSTM & GloVe  & 0.   & 0. & 0.838\\
		Deep CNN & ElMo & 0.   & 0. & 0.860\\
		Bidirectional LSTM & ElMo & 0.   & 0. & \textbf{0.862}\\
		\hline
	\end{tabular}
\end{center}
\textit{*S} refers to sentiment features and \textit{T} refers to topic features
...
\begin{center}
	\begin{tabular}{ |c||c|c|c|c|}
		...
		\textbf{Machine Learning Model}& \textbf{Feature(s)*} & \textbf{Precision} & \textbf{Recall} & \textbf{$\mathbf{F_1}$ Score}\\
		\hline\hline
		Gaussian Na\"{i}ve Bayes & Bag of Words & 0.   & 0. & 0.681\\
		Random Forest Classifier & GloVe  & 0.   & 0. & 0.645\\
		Support Vector Machine & ElMo  & 0.   & 0. & 0.754\\
		Logistic Regression & ElMo  & 0.   & 0. & 0.762\\
		Logistic Regression & ElMo + S + T + P & 0.   & 0. & \textbf{0.790}\\
		\hline\hline
		\textbf{Deep Learning Model}& \textbf{Feature(s)} & \textbf{Precision} & \textbf{Recall} & \textbf{$\mathbf{F_1}$ Score}\\
		\hline
		Vanilla RNN & ElMo  & 0.   & 0. & 0.817\\
		Bidirectional LSTM & GloVe  & 0.   & 0. & 0.822\\
		CNN & ElMo  & 0.   & 0. & 0.832\\
		Bidirectional LSTM & ElMo  & 0.   & 0. & \textbf{0.849}\\
		Long Short-Term Memory & GloVe & 0.   & 0. & 0.841\\
		\hline
	\end{tabular}
\end{center}
