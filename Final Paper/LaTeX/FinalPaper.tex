...
\begin{figure}[H]
	\begin{center}
		\includegraphics[width=0.7\textwidth]{Images/absolute_differences.png}
...
We found that sentiment features are objectively useful predictors of sarcasm in Twitter data and News Headlines, as logistic regression models trained on these features achieved $F_1$ scores of 0.593 and 0.516, respectively. We found that sentiment features were most informative when extracted from Twitter data as opposed to News Headlines, and we hypothesise that this is due to the use of more overtly sarcastic expressions, such as "\textit{i love working weekends! \#not}".



As these scores exceed 0.5, this indicates that their performance bettered that of a trivial classifier which predicts class labels with apparent randomness. Hence, we conclude that sentiment features are sufficiently informative so as to guide a model to make non-trivial predictions. 
...
\ Hence, we conclude that sentiment features are sufficiently informative so as to guide a model to make non-trivial predictions. Secondly, 

we hypothesise that 

Secondly, conventional punctuation features 

We found that sentiment features can be useful predictors of sarcasm when extracted from Twitter data, as a logistic regression model trained on these features achieved an $F_1$ score of 0.593. While sentiment features were less informative when extracted from News headlines, the logistic regression model trained on these features achieved 0.516 - 1.6\% greater than the average expected score of a na\"{i}ve model which classifies with apparent randomness.

; however, sentiment features extracted from News Headlines showed poor performance in comparison. 


 below the expected standard (0.5) of a naive classifier whene. As such, we consider sentiment features 


% they performed below the expected standard (0.5) of a non-sophisticated model that classifies a balanced dataset with apparent randomness.

more conventional punctuation features were  
