...
\noindent As the feature vectors are small ($1 \times 6$ dimensions), we use logistic regression for this task. We train three logistic regressors (one for each dataset) separately using sentiment features only - this enables us to assess whether sentiment features alone are indicative of sarcasm in different media types. We then use 5-fold cross-validation to compute the $F_1$ scores of the trained models.
...
\begin{center}
	\begin{tabular}{ |p{4.7cm}||p{1.2cm}|p{1.5cm}|p{1.2cm}|p{1.2cm}|  }
		...
		Logistic Regression &   0. & 0.   & 0. & \textbf{0.}\\
		Random Forest Classifier &0. & 0. &  0. & 0.\\
		Gaussian Naive Bayes  & 0.  & 0.&  0. &0.\\
		K-Means& 0.  & 0.   & 0. & 0.\\
		\hline
	\end{tabular}
\end{center}
