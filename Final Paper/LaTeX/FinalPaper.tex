...
\noindent Five classifiers are trained independently on each of our datasets, vectorised using Bag of Words, TF-IDF, GloVe and ElMo. We separate the results into two tables to allow us to contrast the results of traditional vectorisation approaches (\textit{figure \_}) with embedding approaches (\textit{figure \_}). The $F_{1}$ scores of the trained models are provided to 3 significant figures, and the scores of the best-performing models are highlighted for each dataset, paired with each vectorisation approach.\\
...
\begin{center}
	\textbf{Figure -}: $F_1$ scores of machine learning classifiers, trained on Bag of Words and TF-IDF vectors
\end{center}
...
\begin{center}
	\textbf{Figure -}: $F_1$ scores of machine learning classifiers, trained on GloVe and ElMo vectors
\end{center}
...
\begin{center}
	\textbf{Figure -}: $F_1$ scores of Machine Learning classifiers when trained on handcrafted features
\end{center}

\begin{center}
	\begin{tabular}{ |p{3cm}||p{5.4cm}|p{3.7cm}|p{2cm}|  }
		\hline
		\textbf{Dataset} & \textbf{Best Model Architecture}& \textbf{Additional Features} & \textbf{Improved $\mathbf{F_1}$ Score}\\
		\hline\hline
